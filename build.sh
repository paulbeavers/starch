...
install -d "$DEST"
for item in install.sh README.md config starch-config splash; do
    cp -r "$REPO/$item" "$DEST/"
done
...
# ── boot splash ───────────────────────────────────────────────────────────────
# The theme goes into the airootfs directly rather than being left to
# install.sh, because the *live* boot has to splash too and install.sh does not
# run until the install is nearly over. Putting it here also means the clone
# inherits it: the installed system is a copy of this filesystem, so the theme
# and the choice of theme come across without being installed twice.
PLY="$AIR/usr/share/plymouth/themes/starch"
install -d "$PLY"
cp "$REPO/splash/theme/"* "$PLY/"
...
# compositor paints, which on a slow machine is a second of black. With it the
# last frame stays in the framebuffer, and installer-session paints the same
# logo on the same colour over the top, so the seam does not show.
...
# the monitor. This used to be cage, which maximizes any toplevel with no
# parent — so branding.desc's windowSize could never take effect and on a 5K
# display Calamares was 5120 pixels wide. The compositor runs as the live user
# and elevates Calamares itself; Hyprland refuses to run as root, and is right
# to. cage is kept as the fallback: if the session fails for any reason, a
# full-screen installer is much better than none.
#
...
if [[ $XDG_VTNR == 1 && -z ${STARCH_SHELL:-} ]]; then
    ...
    # out what this display wants from its physical size. installer-session
    # passes it to Calamares itself; the fallback has to be told.
    /usr/local/lib/starch/installer-session \
